						...
						<form role="form" name="newCommentForm" novalidate>
							...
							<div class="form-group">
								<input type="text" class="form-control" required="required"
									ng-model="comment.name" name="name" placeholder="Name" />
								<div class="MarginTop_5">
									<span class="alert alert-danger Padding_5"
										ng-show="displayValidationError && newCommentForm.name.$error.required">
							...
							<div class="form-group">
								<input type="text" class="form-control" required="required"
									ng-model="comment.mail" name="mail"
									placeholder="<spring:message code='sample.email'/> " />
								<div class="MarginTop_5">
									<span class="alert alert-danger Padding_5"
										ng-show="displayValidationError && newCommentForm.mail.$error.required">
										<spring:message code="required" />
									</span>
								</div>
							</div>
							<div class="form-group">
								<textarea class="form-control" required="required"
									ng-model="comment.commentDesc" name="commentDesc" rows="3"></textarea>
								<div class="MarginTop_5">
									<span class="alert alert-danger Padding_5"
										ng-show="displayValidationError && newCommentForm.commentDesc.$error.required">
										<spring:message code="required" />
									</span>
								</div>
							</div>
							<button type="submit" class="btn btn-primary"
								ng-click="createComment(newCommentForm,'${selectedArticle.id}');">Submit</button>
						</form>
					</div>
						...
						<div ng-class="{'hidden': comment.enabled != 1}">
							...
							<div class="media-body">
								<h4 class="media-heading">
									{{comment.name}} <small>{{comment.createdDate}}</small>
								</h4>
								{{comment.commentDesc}}
							</div>
						</div>
